...
```{r PDI_response_surfaces,fig.cap='PDI response surfaces for simulated descriptor values from trained Random Forest models. All other descriptors were set at the values of those of the trees with the lowest PDI used for trainig the models',cache=TRUE}
source('../code/PDI_response_surfaces.R')
grid.arrange(PDI_response_plot$`a)`,PDI_response_plot$`b)`,PDI_response_plot$`c)`,PDI_response_plot$legend,nrow = 2)
```
...
grid.arrange(DAI_response_plot$`a)`,DAI_response_plot$`b)`,DAI_response_plot$`c)`,DAI_response_plot$`d)`,DAI_response_plot$`e)`,DAI_response_plot$`f)`,DAI_response_plot$legend,ncol = 3,layout_matrix = rbind(c(1,4,7),c(2,5,7),c(3,6,7)),widths = c(4,4,1))
```