...
A severely declining oak is most prominantly characterised by very poor crown condition and numerous dead branches (ref).
...
A popular algroith is that of random forest due to it's versatility, relatively trivial training requirements and resiliance against overfitting [@breiman2001] (refs). 
It is an ensemble machine learning method where forests of bootstrap sampled decision trees are grown, yielding both out of bag (OOB) sample prediction accuracies and variable importance.
...
- Crown assessments
- Cardinal assessments
- Stem assessments - 2m base, 3m base

Phenotypic data were collected from a total of 174 trees from 9 sites from across the south, east and west of England (Figure #).
...
Cardinal crown radius measurements were averaged.

### Machine learning analyses

The code used to generate the decline indexes is available as an R package at [https://github.com/jasenfinch/pdi].
...
The phenotypic descriptors were analysed using 100 repeatitions of unsupervised random forest.
The proximity 
...
Proximity matrix transformed to a disimilarity (1- P) then multidimensional scaling (MDS) to 2 dimensions applied.
The first dimension was found to be related to the overall severity of decline. 
It was min-max scaled to give the PDI a scale between 0 and 1 with more severly declining trees having a score closer to 1.
