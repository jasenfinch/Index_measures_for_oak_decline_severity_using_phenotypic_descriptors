...
Local Interpretable Model-Agnostic Explanations (LIME) To understand the descriptors contributing to case specific examples of each of the indexes,  were computed for a range of specific cases [@ribeiro2016]. 
The lime R package v`r packageVersion('lime') %>% as.character()` was used with the default parameters [@pedersen2019].
A low, a moderate and a severe decline cases was used for the PDI. For each case, a tree with a score closest to 0, 0.5 and 1 respectively was selected.
For the DAI, a decline neautral case was selected with a score closes to 0.
Then for each of the AOD and COD syndrome types a moderatefor the DAI . 
