...
Oak decline is becoming/predicted to become increasingly prevalent in the UK and across Europe with the onset of climate change (ref).
...
Also a chronically declining oak (COD) can subsequently begin to develop the stem bleeds associated with AOD.
...
There is a growning need for sensitive, continuous measures of tree decline that accuratly reflect the decline spectrum [@pontius2014].
This has the potential to greatly imporove monitoring to enable the detection of decline as well as it's spread and severity over time to enable accurate forecasting and inform forestry management practices (ref).
...
The remission group included trees that had shown stem bleeds associated with AOD in the past but were not showing signs of stem bleeds at the time of assessment.

DBH (DBH) was collected at a stem height of 1.3m, to the nearest 0.1cm.
...
These descriptors are shown in Supplementary Table 1.
...
To account age, size and site specific tree difference, an adjustment of site means to the overall mean was applied to each of the size descriptors (DBH, lower crown height, timber height, total height, crown radius). 
...
High discrimination was found in the phenotypic descriptors between between the sites using supervised random forest classification (Supplementary  figure 3a; margin = `r site_rf_margin$Margin %>% signif(digits = 3)`). 
...
The top four ranked descriptors found to contribute to the PDI index model were all crown size and condition related descriptors with crown volume the top ranked descriptor overall (Figure 2a).
Other tree size related descriptors such as and stem bleeding descriptors such as contributed secondarily.
...
For instance, declining trees were found to have an estimated crown volume below 841m^3^.
...
Bleed prevalence and stem symptom descriptors were not found to be important to any of the decline type cases.
...
Crown condition descriptors such as crown volume and crown condition contributed highly to the PDI (Figure 2a).
...
It can be seen in Figure 3 that as the percentage missing crown and percentage crown transparency increase and therefore crown condition and estimated crown volume decrease, the PDI increases. 
...
Scaling this technique to enable generalised naive compuation of decline indexes for any tree sampled in the UK would require a substantially larger training data set that incorporates the descirptive diversity of oak decline in the population as whole (ref).
Scaling to this level would also require an approprite method of tree size correction that could be applied on an individual tree basis.
A potential solution to this would be to correct trees based on estimated age from DBH as proposed by @white1998.
This would enable the development of web based tools that can then provide acces to 
...
- Aid long term monitoring by providing comparibility over time. This was highlighted in the recently published Action Oak Knowledge Review as a key requirment in the future for safeguarding oak health. [@quine2019],
...
The approach presented here is easily adaptable to incorporate any phenot
