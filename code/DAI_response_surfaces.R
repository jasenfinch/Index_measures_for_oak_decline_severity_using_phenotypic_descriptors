...
# spectrumTrees_DAI$`a)`$`Active bleed size (cm)` <- ranges$min[ranges$Descriptor == 'Active bleed size (cm)']
spectrumTrees_DAI$`a)`$`Active bleed size (cm)` <- 1 / 4
spectrumTrees_DAI$`b)`$`Active bleed size (cm)` <- ranges$center[ranges$Descriptor == 'Active bleed size (cm)'] / 4 
spectrumTrees_DAI$`c)`$`Active bleed size (cm)` <- ranges$max[ranges$Descriptor == 'Active bleed size (cm)'] / 4
...
# spectrumTrees_DAI$`a)`$`Black staining size (cm)` <- ranges$min[ranges$Descriptor == 'Black staining size (cm)']
spectrumTrees_DAI$`a)`$`Black staining size (cm)` <- 1 / 4
spectrumTrees_DAI$`b)`$`Black staining size (cm)` <- ranges$center[ranges$Descriptor == 'Black staining size (cm)'] / 4
spectrumTrees_DAI$`c)`$`Black staining size (cm)` <- ranges$max[ranges$Descriptor == 'Black staining size (cm)'] / 4
...
plotRanges_DAI_abc <- spectrumTrees_DAI[c('a)','b)','c)')] %>%
  ...
  map(~{
    type <- .
    list(
      `Active bleeds` = seq(ranges$min[ranges$Descriptor == 'Active bleeds'],ranges$center[ranges$Descriptor == 'Active bleeds'] / 4,length.out = 100),
      `Black staining` = seq(ranges$min[ranges$Descriptor == 'Black staining'],ranges$center[ranges$Descriptor == 'Black staining'] / 4,length.out = 100)
    ) %>%
      expand.grid() %>%
      as_tibble() %>%
      mutate(ID = 1) %>%
      left_join(spectrumTrees_DAI[[type]] %>%
                  select(-`Active bleeds`,-`Black staining`) %>%
                  mutate(ID = 1),
                by = 'ID') %>%
      select(-ID) %>%
      mutate(
...
DAI_response_plot_abc <- plotRanges_DAI_abc %>%
  ...
  map(~{
    type <- .
    pl <- plotRanges_DAI_abc[[type]] %>%
      ggplot(aes(x = `Active bleeds`,y = `Black staining`,fill = DAI,z = DAI)) +
      geom_raster() +
      geom_raster() +
      geom_contour(colour = 'black',binwidth = 0.1) +
      geom_text_contour(stroke = 0.2,binwidth = 0.1,min.size = 10,size = 3) +
      ...
      labs(title = type,
           caption = str_c(
             'Active bleed size (cm) = ',plotRanges_DAI_abc[[type]]$`Active bleed size (cm)`[1] %>% round(3),'\n',
             'Black staining size (cm) = ',plotRanges_DAI_abc[[type]]$`Black staining size (cm)`[1] %>% round(3),'\n',
             'Diameter at breast height (cm) = ',plotRanges_DAI_abc[[type]]$`Diameter at breast height (cm)`[1] %>% round(3)
           )
      )
    return(pl)
